...
# Constants
UART_DEVICE = "/dev/pts/9" # Change to actual serial port
BAUDRATE = 921600
PACKET_SIZE = 20
...
# TODO: Fix recv packet format
# Function to listen for IMU broadcast
def listen_for_broadcast():
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind(("127.0.0.1", BROADCAST_PORT))
    print("Listening for IMU broadcast...")
    while True:
        data, _ = sock.recvfrom(PACKET_SIZE)
        packet_count, x, y, z = struct.unpack("!Ifff", data)
        print(f"Received Packet: Count={packet_count}, X={x}, Y={y}, Z={z}")
